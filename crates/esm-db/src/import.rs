...
pub fn build_teams_from_datapack(pack: &DataPack) -> Vec<Team> {
    ...
    for td in &pack.teams {
        let roster: Vec<Player> = get_full_roster(pack, &td.name)
            .into_iter()
            .map(|p| {
...
pub fn build_moba_teams_from_datapack(pack: &DataPack) -> Vec<MobaTeam> {
    ...
    for td in &pack.teams {
        let players: Vec<MobaPlayer> = get_full_roster(pack, &td.name)
            .into_iter()
            .map(|p| {
...
fn get_full_roster(pack: &DataPack, team_name: &str) -> Vec<PlayerData> {
    let mut team_players: Vec<PlayerData> = pack
        .players
        .iter()
        .filter(|p| p.team == team_name)
        .cloned()
        .collect();
    let required_roles = ["Top", "Jungle", "Mid", "Bot", "Support"];
    for role in required_roles {
        if !team_players.iter().any(|p| p.role == *role) {
            team_players.push(generate_player(pack, role, team_name));
        }
    }
    team_players
}
...
impl DataPackImporter {
    ...
    fn insert_players(
        ...
        for td in &pack.teams {
            let team_id = team_ids.get(&td.name).copied();
            for player in get_full_roster(pack, &td.name) {
                conn.execute(
