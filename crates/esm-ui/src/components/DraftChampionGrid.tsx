...
export function DraftChampionGrid({
  ...
  }, [availableChampions, championDetails, searchText, classFilter, roleFilter]);
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="flex items-center gap-1.5 px-2 py-1 rounded-lg border flex-1 min-w-56"
          style={{
          ...
          {ALL_ROLES.map((role) => {
            ...
            return (
              <button
                ...
                className="px-2 py-0.5 rounded text-[0.6rem] font-bold tracking-wide transition-all duration-150 border"
                style={{
                  backgroundColor: isActive ? 'rgba(16,185,129,0.15)' : 'transparent',
                  borderColor: isActive ? 'var(--color-accent-emerald)' : 'var(--border-subtle)',
                  color: isActive ? 'var(--color-accent-emerald)' : 'var(--text-muted)',
                }}
              >
                {roleShort(role)}
              </button>
          ...
          {filteredChampions.map((champ) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-stretch gap-2 p-2.5 rounded-xl text-left transition-all duration-150 border min-h-[132px]"
                style={{
                  backgroundColor: isHovered ? 'rgba(6,182,212,0.14)' : 'var(--bg-elevated)',
                  borderColor: isHovered ? 'var(--color-accent-cyan)' : 'var(--border-subtle)',
                  color: 'var(--text-primary)',
                  opacity: isPlayerTurn ? 1 : 0.5,
                  cursor: isPlayerTurn ? 'pointer' : 'not-allowed',
                  boxShadow: isHovered ? '0 0 12px rgba(6,182,212,0.2)' : 'none',
                }}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-bold text-sm leading-tight truncate">{champ}</div>
                    {info?.tags?.length ? (
                      <div
                  ...
                  {info?.meta_tier ? (
                    <span
                      className="px-1.5 py-0.5 rounded text-[0.65rem] font-black shrink-0"
                      style={{
                ...
                <div className="flex flex-wrap gap-1 min-h-[22px]">
                  {info?.preferred_roles?.length ? info.preferred_roles.map((role: string) => (
                    <span
                      key={`${champ}-${role}`}
                      className="px-1.5 py-0.5 rounded text-[0.55rem] font-bold tracking-wide"
                      style={{
                ...
                {info ? (
                  <div className="flex flex-wrap gap-1.5 mt-auto">
                    <span
                      className="px-1.5 py-0.5 rounded text-[0.55rem] font-semibold"
                      style={{ backgroundColor: `${classColor(info.class)}18`, color: classColor(info.class) }}
                    >
                      {info.class}
                    </span>
                    <span
                      className="px-1.5 py-0.5 rounded text-[0.55rem] font-semibold"
                      style={{ backgroundColor: `${scalingColor(info.scaling)}18`, color: scalingColor(info.scaling) }}
                    >
                      {info.scaling}
                    </span>
                    {info.best_mastery ? (
                      <span
                        ...
                        style={{ backgroundColor: `${masteryColor(info.best_mastery)}18`, color: masteryColor(info.best_mastery) }}
                      >
